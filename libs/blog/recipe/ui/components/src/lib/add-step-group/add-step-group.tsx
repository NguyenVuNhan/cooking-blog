...
export function AddStepGroup(props: AddStepGroupProps) {
  ...
  return <>
    {fields.map((step, index) => (
      ...
      >
        <Grid item sm={12} className="d-flex align-items-center">
          ...
          <IconButton aria-label="delete" onClick={removeStep(index)} size="large">
            <DeleteIcon color="error" fontSize="small" />
          </IconButton>
          {index > 0 && (
            <IconButton onClick={() => move(index, index - 1)} color="secondary" size="large">
              <ArrowUpwardIcon color="primary" fontSize="small" />
            </IconButton>
          )}
          {index < fields.length - 1 && (
            <IconButton onClick={() => move(index, index + 1)} size="large">
    ...
    <Grid item container alignItems="center" justifyContent="center" sm={12}>
